...
var_discount_2 = 1000                     #Secon1d discount starts.
    ...
    while True:                                            #Repeat Menu until stops.
        ...
        if (len(input_1) == 1):                                           #Checking input length.
            if (input_1 == 'O'):                                          #If input is "O".
                ...
                print("\n" * 2 + "ERROR: Invalid Input (" + str(input_1) + "). Try again!")       #Invalid input.
        else:                                                                                     #If input length not equal to 1...
            print("\n" * 2 + "ERROR: Invalid Input (" + str(input_1) + "). Try again!")           #Invalid input.

def def_order_menu():                                                                               #yourself
    while True:                                             # While looping to keep menu alive
        ...
        if len(input_1) == 1:
            if (input_1 == 'F'):  #Easy Access Checking Logic
                ...
                print("\n" * 5 + "ERROR: Invalid Input (" + str(input_1) + "). Try again!") # Handling Bad Inputs
        else:
...
def def_full_file_reader():                                                                         
    ...
    i = 0
    while i <= (len(list_foods) - 1):  #Enumerate through food list to filter out prices and setup print Formatting by replacing spaces with count difference of string length and align Prices to the most left of the terminal
...
def def_file_sorter(): # Applying Sorting to the array to be sorted from A-Z ASC ((AND)) Extracting out prices after sorting and appending them to a prices array accordingly to a parallel indexes
    ...
    while i < len(list_services):
        list_item_price[80 + i] = float(list_services[i][int(list_services[i].index("RM") + 3):]) # Applying extraction on 80 and above items which are Services
        i += 1
def_file_sorter()
    ...
    while True:
            ...
            try:        #Cautions Error Handling to prevent program crashing and hand out exceptions as a readable error to notify user
                ...
                if ((int(input_1) <= len(list_foods) and int(input_1) > 0) or (int(input_1) <= len(list_drinks) + 40 and int(input_1) > 40)):
                     try:
                        print("\n" + "_" * 72 + "\n" + str(list_foods[int(input_1) - 1])) # Handling Food Selection / The try/Except to handle out of index error as if it  not exists in the array
                     except:
                        pass
                     try:
                         print("\n" + "_" * 72 + "\n" + str(list_drinks[int(input_1) - 41])) # Handling Drinks Selection / The try/Except to handle out of index error as if it  not exists in the array
                     except:
                        pass
...
def def_other_services():
    while True:
        ...
        i = 0
        while i < len(list_services):
            print(" (" + str(81+ i) + ")" + " " + str(list_services[i])) # Services starts from 81 + and now it is being enumerated into a list.
...
def def_payment():
    while True:
        ...
        i = 0
        while i < len(list_item_order): #Enumerating order array items and summing up its prices * quantities
